...
describe("runClean", () => {
  ...
  it("allows CLI include override", async () => {
    ...
    await runClean(config, { command: "turbo", include: [".custom"] });
  ...
  it("supports tag-intersect when command and tags are both passed", async () => {
    ...
    const config = makeConfig();
    config.commands.turbo.include = [".turbo"];
    await runClean(config, { command: "turbo", tags: ["cache"], tagIntersect: true });
  ...
  it("writes json report when report=json", async () => {
    ...
    try {
      await runClean(config, { command: "turbo", report: "json" });
    } finally {
      console.log = originalLog;
    }

    const json = logs.find((line) => line.includes("\"removedTargets\""));
  ...
  it("fails with available commands when command is unknown", async () => {
    ...
    }).rejects.toThrow('Unknown command "abcd". Available commands: turbo, wrangler');
  });
