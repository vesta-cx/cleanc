...
const normalizePattern = (pattern: string): string => toPosix(pattern.trim()).replace(/^\.\/+/, "");

const resolveRoot = (configDir: string, baseCwd?: string, cliCwd?: string): string => {
  const root = cliCwd ?? (baseCwd ? path.resolve(configDir, baseCwd) : configDir);
...
const assertInsideRoot = (target: string, root: string): void => {
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    throw new Error(`Path traversal detected: ${target} attempts to escape ${root}`);
  }
};
...
const expandTargets = async (root: string, include: string[], exclude: string[]): Promise<FileEntry[]> => {
  ...
  for (const pattern of include.map(normalizePattern).filter(Boolean)) {
    ...
    if (absLiteral !== root) {
      ...
      try {
        ...
        const rel = toPosix(path.relative(root, absLiteral));
        byAbs.set(absLiteral, { abs: absLiteral, rel, isDir: stats.isDirectory() });
      } catch {
  ...
  const filtered = [...byAbs.values()].filter((entry) => !excluded.some((pattern) => matchesPattern(entry.rel, pattern)));
  return filtered;
};

const isProtected = (rel: string, protect: string[]): boolean => protect.some((pattern) => matchesPattern(rel, pattern));
...
const mergeEntry = (defaults: NormalizedEntry, entry?: NormalizedEntry): NormalizedEntry => {
...
const applyCliOverrides = (entry: NormalizedEntry, options: RunOptions): NormalizedEntry => ({
  ...entry,
...
const selectCommands = (config: NormalizedConfig, options: RunOptions): string[] => {
  const explicit = resolveCommandAlias(options.command);
  if (explicit && !config.commands[explicit]) {
    const available = Object.keys(config.commands).sort().join(", ") || "(none)";
    throw new Error(`Unknown command "${options.command}". Available commands: ${available}`);
  }

  const requestedTags = (options.tags ?? []).map((tag) => tag.trim()).filter(Boolean);
  const matchesByTag =
    requestedTags.length === 0
      ? []
      : Object.entries(config.commands)
          .filter(([, entry]) => entry.tags.some((tag) => requestedTags.includes(tag)))
          .map(([name]) => name);

  if (!explicit && requestedTags.length === 0) return [];
  if (explicit && requestedTags.length === 0) return [explicit];
  if (!explicit && requestedTags.length > 0) return [...new Set(matchesByTag)];
...
const removeTarget = async (entry: FileEntry, mode: CleanMode): Promise<void> => {
  if (mode === "delete") {
  ...
  await Promise.all(children.map((child) => rm(path.join(entry.abs, child), { recursive: true, force: true })));
};
...
export async function runClean(
  ...
  for (const commandName of runs) {
    const merged = mergeEntry(config.defaults, commandName ? config.commands[commandName] : undefined);
    const effective = applyCliOverrides(merged, options);

    if (!effective.enabled) {
      if (effective.verbose) {
        console.log(`Skipping ${commandName ?? "global"} (enabled=false)`);
      }
      continue;
    }

    const root = resolveRoot(config.configDir, effective.cwd, options.cwd);
    const targets = await expandTargets(root, effective.include, effective.exclude);
    summary.matchedTargets += targets.length;

    const protectedTargets = targets.filter((target) => isProtected(target.rel, effective.protect));
    summary.skippedProtected += protectedTargets.length;
    const removable = targets.filter((target) => !isProtected(target.rel, effective.protect));

    if (effective.confirm) {
      if (!effective.force) {
        if (!effective.interactive) {
          throw new Error("Confirmation required but interactive mode is disabled. Use --interactive or --force.");
        }
        const ok = await promptConfirm(`Delete ${removable.length} matched targets for ${commandName ?? "global"}?`);
        if (!ok) {
    ...
    const deleteOne = async (target: FileEntry): Promise<void> => {
      if (effective.dryRun) {
        if (effective.verbose) {
          console.log(`[dryRun] would remove: ${target.rel}`);
        }
        return;
      }
    ...
    try {
      ...
      if (effective.verbose) {
        console.error(`Ignored error (${commandName ?? "global"}): ${message}`);
      }
    }
  }
  ...
  if (summary.errors.length > 0 && !(options.ignoreErrors ?? config.defaults.ignoreErrors)) {
    throw new Error(`cleanc completed with ${summary.errors.length} error(s).`);
  }
}
